...
#Declaring variables
defenseReductionMultiplier = 0.06   ##Every point of defense is equal to 6% effective HP increase. (Same with wc3 and dota) Due to change in the future.
speedModifier = 1.5   ##Changes combat speed, higher = slower
...
none = weapons('none', 0, 0, 'none', 0, 0)
stick = weapons('stick', 1, 0, '''nature's fury''', 0.1, 1)
pitchfork = weapons('pitchfork', 3, 1, 'none', 0.1, 2)
rustySword = weapons('rusty sword', 5, 0, 'tetanus', 0.1, 2)
...
def loot(lootLevel, lootMessage):
    for i in weaponList:
        if hero.level >= i.level:
            totalDropRate += i.dropRate
    if lootMessage == 'combatLoot':
        print ('You find PLACEHOLDER on the dead body of your enemy.')
    elif lootMessage == 'exploreLoot':
        print ('You stumbled upon an ancient treasure.')
    else:
