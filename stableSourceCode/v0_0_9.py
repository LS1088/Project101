...
defenceReductionMultiplier = 0.06   #Every point of defence is equal to 6% effective HP increase.
...
lootChance = 1
inventory = {'giant hammer':1, 'potion':1, 'strong potion':1, 'stick':1}
...
def checkStats():
    ...
    if hasattr(hero.weapon, 'attack'):
        print('Weapon attack:', hero.weapon.attack)
    if hasattr(hero.weapon, 'defence'):
        print('Weapon defence:', hero.weapon.defence)
    if hasattr(hero.weapon, 'specialEffect'):
        print('Special Effect:', hero.weapon.specialEffect)
    print(continueStr)
    input()
...
def checkInventory():
    ...
    print("Inventory")
    print(line)
    print("Gold:", gold)        #Displays inventory
    if len(inventory) == 0:
        ...
        for key, value in inventory.items():
            if itemNameDict[key].__class__ == hpPotion:
                ...
                print(value, "X", Fore.YELLOW + Style.BRIGHT + itemNameDict[key].name + Fore.RESET + Style.RESET_ALL, end = " | ")
                if hasattr(itemNameDict[key], 'attack'): 
                    print('Attack', itemNameDict[key].attack, end = " ")
                if hasattr(itemNameDict[key], 'defence'):  
                    print('Defence:', itemNameDict[key].defence, end = " ")
                if hasattr(itemNameDict[key], 'specialEffect'):
                    print('Special Effect:', itemNameDict[key].specialEffect, end = " ")
                print("")
        print(line)
        print("Enter the name of the item you wish to use. Input nothing to go back.")      #Using or equipping items
        while useItem != '':
            useItem = input()
            for key, value in inventory.items():
                itemCheckTrigger = 0
                if useItem == key:
                    if key in hpPotionNameDict:         #Consume a potion
                    ...
                    elif key in weaponNameDict:         #Equipment
                        if hasattr(hero.weapon, 'attack'):          #Removes old weapon stats from hero
                        ...
                        if hasattr(itemNameDict[key], 'attack'):        #Adds new weapon stats to hero
...
#SOUND
def bgm():
    pygame.mixer.music.load("moonlight_sonata.ogg")    
